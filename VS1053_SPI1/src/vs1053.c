...
{
  //MP3_DREQ_IN = GPIOF->IDR & 0x0800;
 // while(!MP3_DREQ_IN) ; //Wait for DREQ to go high indicating IC is available
   while(!(GPIOF->IDR & 0x0800));
  MP3_XCS(LOW); //Select control

  //SCI consists of instruction byte, address byte, and 16-bit data word.
  spi1Write(0x02); //Write instruction
  spi1Write(addressbyte);
  spi1Write(highbyte);
  spi1Write(lowbyte);
    
  while(!(GPIOF->IDR & 0x0800)) ; //Wait for DREQ to go high indicating command is complete
...
void vs1053_WriteRegisterW(unsigned char addressbyte, unsigned int data)
{
  ...
  //MP3_DREQ_IN = GPIOF->IDR & 0x0800;
  while(!(GPIOF->IDR & 0x0800)) ; //Wait for DREQ to go high indicating IC is available
  MP3_XCS(LOW); //Select control
...
unsigned int vs1053_ReadRegister (unsigned char addressbyte)
{
  ...
  //SCI consists of instruction byte, address byte, and 16-bit data word.
  spi1Write(0x03); //Read instruction
  spi1Write(addressbyte);

  char response1 = spi1Write(0xFF); //Read the first byte
  
  while(!(GPIOF->IDR & 0x0800)) ; //Wait for DREQ to go high indicating command is complete
  char response2 = spi1Write(0xFF); //Read the second byte
  while(!(GPIOF->IDR & 0x0800)) ; //Wait for DREQ to go high indicating command is complete
...
{
  vs1053_WriteRegister(SCI_VOL, leftchannel, rightchannel);

  // check if we can read back the set values, 
  int volValue = vs1053_ReadRegister(SCI_VOL);
  char left = volValue >> 8;
  char right = volValue & 0xFF;
...
VS1053_INFO vs1053_setup()
{
  ...
  // SPI Clock Speed should be set before calling this function

  //Initialize VS1053 chip 
  MP3_XCS(HIGH); //Deselect Control
  MP3_XDCS(HIGH); //Deselect Data

  MP3_RESET(LOW); // hardware reset
  spi1Write(0xFF); // dummy byte
  Delay(10);
  MP3_RESET(HIGH); // bring up VS1053

  Delay(10);

  // Set initial volume (20 = -10dB). If we cannot set volume, assume device not present
  unsigned char volDefault = 20;
  retVal.isPresent = vs1053_SetVolume(volDefault, volDefault); //Set initial volume (20 = -10dB)

  // Default SCI_MODE register value should be 18432 (0x4800).
  // If it reads 65424 (0xFF90) at powerup while other registers read fine, check module XTEST pin (pin 32) and make sure it is tied to IOVDD, or CVDD3 (pin 31)
  // Some module has this fault, floating XTEST pin, causing undefined behaviour.
  // Also check GPIO0 and GPIO1 (pin 33 and 34) and make sure they are connected properly for SPI mode
  // In Midi MODE, GPIO0 is low and GPIO1 is high. 

  // also set SM_RESET (software reset) - bit 2
  unsigned int sciValue = vs1053_ReadRegister(SCI_MODE);
  vs1053_WriteRegisterW(SCI_MODE, sciValue | 0b0000000000000100); // SM_RESET at bit 2, SM_ADPCM at bit 12
  Delay(100);

  //Let's check the status of the VS1053
  retVal.MP3Mode = vs1053_ReadRegister(SCI_MODE);
  retVal.MP3Status = vs1053_ReadRegister(SCI_STATUS);
  retVal.MP3Bass = vs1053_ReadRegister(SCI_BASS);
  retVal.MP3DecodeTime = vs1053_ReadRegister(SCI_DECODE_TIME);
  retVal.MP3AuData = vs1053_ReadRegister(SCI_AUDATA);
  retVal.MP3HDat1 = vs1053_ReadRegister(SCI_HDAT1);
  retVal.MP3HDat0 = vs1053_ReadRegister(SCI_HDAT0);
  retVal.vsVersion = (retVal.MP3Status >> 4) & 0x000F; 

  // old MP3 Clock before speed adjustment
  // retVal.MP3Clock = vs1053_ReadRegister(SCI_CLOCKF);
  ...
  //SPI2CON1 = SPI2CON1 | 0b00010011; // primary scaler 1:1, secondary scaler 4:1. 

  // new MP3 Clock after speed adjustment
  retVal.MP3Clock = vs1053_ReadRegister(SCI_CLOCKF);

  // volume
  int volValue = vs1053_ReadRegister(SCI_VOL);
  retVal.volLeft = volValue >> 8;
  retVal.volRight = volValue & 0xFF;
...
void vs1053_play(unsigned char* data, unsigned int len){
	...
	while(p <= &data[len - 1]) {
		...
		MP3_XDCS(LOW); //Select Data
		spi1Write(*p++); // Send SPI byte
		MP3_XDCS(HIGH); //Deselect Data
	}
	...
	MP3_XDCS(LOW); //Select Data
	for (i = 0 ; i < 2048 ; i++) {
		while(!(GPIOF->IDR & 0x0800)); //If we ever see DREQ low, then we wait here
		spi1Write(0);
	}
	while(!(GPIOF->IDR & 0x0800)) ; //Wait for DREQ to go high indicating transfer is complete
	MP3_XDCS(HIGH); //Deselect Data
}
...
{
	vs1053_play(HelloMP3, sizeof(HelloMP3));
}